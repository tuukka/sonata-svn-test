		...
		if self.view == self.VIEW_FILESYSTEM:
			...
			if self.view_artist_level == 1:
				...
				for item in self.browse_search_album_with_artist_and_year(self.view_artist_artist, self.view_artist_album, year):
					num = self.sanitize_mpdtag(getattr(item, 'disc', '01'), False, 2) + self.sanitize_mpdtag(getattr(item, 'track', '01'), False, 2)
		...
		for item in self.conn.do.search('album', album):
			if item.has_key('album'):
				...
				if album.lower() == item.album.lower():
					list.append(item)
		list.sort(key=lambda x: int(self.sanitize_mpdtag(getattr(x, 'disc', ''), False, 2) + self.sanitize_mpdtag(getattr(x, 'track', '0'), False, 2)))
		return list
			...
			if artist.lower() == item.artist.lower() and album.lower() == item.album.lower():
				if year is None:
					...
					if year != '0000' and item.has_key('date'):
						...
						list.append(item)
		list.sort(key=lambda x: int(self.sanitize_mpdtag(getattr(x, 'disc', ''), True, 2) + self.sanitize_mpdtag(getattr(x, 'track', '0'), True, 2)))
		return list
